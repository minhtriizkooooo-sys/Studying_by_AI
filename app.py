...
game_state = {
    ...
    "pin": None, 
    "is_running": False,
    "stats": {} # Lưu thống kê câu hỏi: { q_idx: { correct: 0, wrong: 0 } }
}
...
def download_template():
    # Tạo file mẫu đúng cấu trúc file của bạn
    data = {
        'Câu hỏi': ['Ví dụ: Thủ đô của Việt Nam là gì?'],
        'Đáp án A': ['Hải Phòng'],
        'Đáp án B': ['Hà Nội'],
        'Đáp án C': ['Đà Nẵng'],
        'Đáp án D': ['TP.HCM'],
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df.to_excel(writer, index=False, sheet_name='CAU HOI')
    output.seek(0)
    return send_file(output, as_attachment=True, download_name="TEMPLATE_CAU_HOI.xlsx")
...
def handle_upload(data):
    try:
        ...
        emit('error', {'msg': "Lỗi file Excel! Hãy dùng file mẫu."})
...
def start_round():
    ...
    game_state.update({"current_round_qs": selected, "active_q_idx": 0, "is_running": True, "stats": {}})
    send_q()
...
def handle_sub(data):
    ...
    user_choice = str(data['ans']).strip()
    raw_correct_key = str(q['Đáp án đúng']).strip().upper()
    correct_content = str(q.get(f'Đáp án {raw_correct_key}', raw_correct_key)).strip()
    
    is_correct = (user_choice.lower() == correct_content.lower())
    
    # Cập nhật thống kê cho Host
    ...
    if is_correct:
        roll = random.random()
        if roll > 0.70: # Tăng tỷ lệ lên 30% để dễ thấy
            base_pts *= 2
    ...
    p['total'] += base_pts
    p['history'].append({"idx": q_idx+1, "q": q['Câu hỏi'], "u": user_choice, "c": correct_content, "pts": base_pts, "ex": q.get('Giải thích',''), "event": event_msg})
...
def get_host_review():
    # Host review: câu hỏi + số người đúng/sai
    report = []
    for i, q in enumerate(game_state['current_round_qs']):
        stat = game_state['stats'].get(i, {"correct": 0, "wrong": 0})
        report.append({"idx": i+1, "q": q['Câu hỏi'], "c": stat['correct'], "w": stat['wrong']})
