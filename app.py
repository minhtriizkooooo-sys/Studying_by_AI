...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'marie_curie_v2026'
socketio = SocketIO(app, cors_allowed_origins="*", async_mode='gevent')

game_state = {
    "all_questions": [], "used_q_indices": set(), "current_round_qs": [],
    "players": {}, "active_q_idx": -1, "current_round_num": 0,
    "start_time": 0, "pin": None, "is_running": False, "king_sid": None, "current_answers": {}
}
...
def start_round():
    ...
    game_state['active_q_idx'] = 0; game_state['is_running'] = True
    send_q()
...
def send_q():
    ...
    if idx < 10 and game_state['is_running']:
        ...
        socketio.sleep(15.7)
        if game_state['is_running'] and game_state['active_q_idx'] == idx: process_end()
...
def process_end():
    ...
    if idx >= 1 and corrects:
        ...
        if game_state['king_sid'] == fastest:
            bonus = random.choice([50, 100])
            game_state['players'][fastest]['total'] += bonus
            emit('special_event', {'msg': f"🌟 LUCKY SPIN: {game_state['players'][fastest]['name']} +{bonus}đ!"}, broadcast=True)
        elif game_state['king_sid']:
            k = game_state['king_sid']
            stolen = int(game_state['players'][k]['total'] * 0.1)
            game_state['players'][k]['total'] -= stolen
            game_state['players'][fastest]['total'] += stolen
            emit('special_event', {'msg': f"⚡ STEAL: {game_state['players'][fastest]['name']} lấy {stolen}đ từ {game_state['players'][k]['name']}!"}, broadcast=True)
    if game_state['players']:
        game_state['king_sid'] = max(game_state['players'], key=lambda x: game_state['players'][x]['total'])
    game_state['active_q_idx'] += 1; update_lb()
    socketio.sleep(2)
    if game_state['active_q_idx'] < 10: send_q()
    else: game_state['is_running'] = False; emit('enable_review', broadcast=True)
...
def handle_sub(data):
    ...
    game_state['players'][sid]['history'].append({"q": q['Câu hỏi'], "u": data['ans'], "c": q['Đáp án đúng'], "ex": q['Giải thích'], "pts": pts})
