...
socket.on('player_waiting', d => {
    ...
    div.innerHTML = `
        ...
        <button class="btn btn-sm btn-success fw-bold" onclick="approvePlayer('${d.sid}', '${d.name}')">DUYỆT</button>
    `;
    waitingList.appendChild(div);
    waitCount.innerText = waitingList.children.length;
});

// Hàm xử lý Duyệt User (Đổi tên hàm để tránh trùng lặp hệ thống)
function approvePlayer(sid, name) {
    ...
    const waitingList = document.getElementById('waiting-list');
    document.getElementById('wait-count').innerText = waitingList.children.length;

    // 2. Thêm vào danh sách đã duyệt (Cột bên phải)
